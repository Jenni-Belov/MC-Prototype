...
public class InterventionAdministrationManagerService {
	...
	@Synchronized
	public int variableL18n(final ObjectId interventionId,
			final String i18nIdentifier, final LString nameWithPlaceholders,
			final LString valueWithPlaceholders, 
			final LString privacyWithPlaceholders,
			final LString accessWithPlaceholders) {

		int updates = 0;

		val messageVariablesOfInterventions = getAllPossibleMessageVariablesOfIntervention(
				interventionId);

		for (val messageVariablesOfIntervention : messageVariablesOfInterventions) {
			val messageVariables = getAllInterventionVariablesOfIntervention(
					messageVariablesOfIntervention.getId());

			for (val messageVariable : messageVariables) {
				if (messageVariable.getI18nIdentifier()
						.equals(i18nIdentifier)) {
					messageVariable
							.setNamePlaceholders(namePlaceholders);
					messageVariable.setValueWithPlaceholders(
							valueWithPlaceholders);
					messageVariable.setPrivacyWithPlaceholders(
							privacyWithPlaceholders);
					messageVariable.setAccessWithPlaceholders(
							accessWithPlaceholders);
					databaseManagerService.saveModelObject(messageVariable);
